...
def run():
    ...
    run a chain, see :meth:`inference.mcmc.init`.
    '''
    global BURNIN,ITER,posterior
...
def gibbsStep():
    ...
    for gbnV in gbnI.samplingVertices.values():        
        #we sample new state    
    ...
    gbnV = random.choice(gbnI.samplingVertices.values())
    #we sample new state 
    gbnV.value = sampleFullConditional(gbnV)
    '''
    ...
    if BLOCKGIBBS:
        ...
        '''
        attrS = random.choice(gbnI.samplingVerticesByAttribute.keys())
        
        # print 'Chosen sampling attr:',attrS.fullname
        
        for dep in attrS.dependenciesParent:
            attrC = dep.child
            if attrC in gbnI.allByAttribute:
                for gbnV in gbnI.allByAttribute[attrC]:                                                            
                    gbnV.parentAssignments()
        
        for gbnV in gbnI.samplingVerticesByAttribute[attrS]:      
        ...
        LAZY STANDARD GIBBS
        '''
        for attrS in gbnI.samplingVerticesByAttribute.keys():
            for dep in attrS.dependenciesParent:
                attrC = dep.child
...
def collectSamples(nSample):
    ...
    """
    global gbnI
    for (i,gbnV) in enumerate(gbnI.eventVertices.values()):                
...
def init(currentGBN,chainID='standardChain'):
    '''
    ...
    #posterior samples
    posterior.initChain(chainID,ITER,gbnI.eventVertices)
...
def initializeVertices():
    '''
    Creates an initial state for the markov chain by assigning a random sample to all sampling vertices
    '''
    for gbnV in gbnI.samplingVertices.values():
...
def configure(prmI):
    ...
    :arg prmI: :class:`.PRM` instance
    '''
    for attr in prmI.attributes.values():
